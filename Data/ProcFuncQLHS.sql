...
BEGIN	
	...
	(	
		SOCV, NOIDUNG, NOIDUNG_Unsign, NGAYCV, MSNV, MSLOAICV, MSCQ, MSGIAIDOAN, MSCVCHA
	)
	VALUES
	( 
		@stSOCV, @stNOIDUNG, dbo.fConvertToUnsign(@stNOIDUNG), @dNGAYCV, @iMSNV, @iMSLOAICV, @iMSCQ, @iMSGIAIDOAN, @iMSCVCHA
	)

	DECLARE @stMSCV NVARCHAR(MAX)	
	SELECT @stMSCV = CONVERT(nvarchar, SCOPE_IDENTITY()) -- Lấy ID của REC mới INSERT và chuyển thành chuỗi
	...
	SET @stFILE_DEST = @stPATHSERVER + N'\' + RTRIM(@stMSCV)

	IF (CHARINDEX('\', @stFILEPDF)>0) 
	BEGIN		
		SET @stFILE_DEST = @stPATHSERVER + dbo.fGetExtFileName(@stFILEPDF)		
		EXEC USP_CopyFileBySQL @stFILEPDF, @stFILE_DEST
	END

	IF (CHARINDEX('\', @stFILEOFFICE)>0) 
	BEGIN		
		SET @stFILE_DEST = @stPATHSERVER + dbo.fGetExtFileName(@stFILEOFFICE)		
		EXEC USP_CopyFileBySQL @stFILEOFFICE, @stFILE_DEST
	END

	IF (CHARINDEX('\', @stFILERAR)>0) 
	BEGIN		
		SET @stFILE_DEST = @stPATHSERVER + dbo.fGetExtFileName(@stFILERAR)		
		EXEC USP_CopyFileBySQL @stFILERAR, @stFILE_DEST
	END
...
BEGIN
	IF EXISTS(SELECT MSNV FROM dbo.tNhanVien WHERE MSNV = @stMSNV)
		UPDATE dbo.tNhanVien SET HOTEN = @stHOTEN, QUYENTC = @stQUYENTC, EMAIL = @stEMAIL, HIEULUC = @bHIEULUC WHERE MSNV = @stMSNV
	ELSE
	BEGIN
		IF EXISTS(SELECT MSNV FROM dbo.tNhanVien WHERE MSNV = 'USER')
			DELETE FROM dbo.tNhanVien WHERE MSNV = 'USER'
		INSERT INTO dbo.tNhanVien (MSNV, HOTEN, MATKHAU, QUYENTC, EMAIL, HIEULUC) VALUES (@stMSNV, @stHOTEN,'2003011414115776479911161271372042013444', @stQUYENTC, @stEMAIL, @bHIEULUC)
	END
END
GO
...
BEGIN
	IF NOT EXISTS(SELECT MSNV FROM dbo.tNhanVien WHERE MSNV = @stMSNV)
		INSERT INTO dbo.tNhanVien (MSNV, HOTEN, QUYENTC, EMAIL, MATKHAU, HIEULUC) VALUES (@stMSNV, @stHOTEN, 'NV', '', '2003011414115776479911161271372042013444',1)
END
GO
